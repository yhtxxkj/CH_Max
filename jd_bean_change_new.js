...
//global.GLOBAL_AGENT.HTTP_PROXY="http://api2.xkdaili.com/tools/XApi.ashx?apikey=XKB4E61548331CFD7233&qty=1&format=txt&split=0&sign=489668d95497513fe4433517ef35398c";
...
!(async () => {
    ...
    //    $.levelName = '';
        ...
        await TotalBean();//检测ck
        //await TotalBean2();//备用检测接口
        //await getJoyBaseInfo();//惊喜狗车
        await getJdZZ();//京东赚赚
        await cash(); //极速金币
        await getMs();//京东秒杀
        await queryScores();//Plus分
        await getjdfruit();//农场
        await getjdfruitinfo(); //农场
        await requestAlgo();//惊喜h5st
        await JxmcGetRequest();//惊喜农场
        await bean();//查询京豆
        await jdCash();//领现金
        await hfjifen();//积分查询
        await rewardCenter();//种豆累积获得
        await GetJoyRuninginfo();//汪汪赛跑
        await health();  //健康社区
        await getDdFactoryInfo(); //京东工厂
       // await getJxFactory(); //京喜工厂
        await getCoupon();//优惠券
        await trialcount();//京东试用
        await trialWaitdraw();//京东试用
        await diandianquan(); //点点券
        await mofang();//小魔方
        await plantBean();// 种豆得豆
        await jxgold(); //惊喜购物金
        await CheckEcard();//E卡查询
	   await signFreeHome();//签到免单
        await showMsg();//发送信息
...
async function showMsg() {
   ...
   ReturnMessage = `【京东账号】${$.nickName || $.UserName}\n`;
    ...
    if ($.JingXiang) {
    	  ...
    	  ReturnMessage += `【温馨提示1】如果查询显示为0，因狗东接口限流，属于正常情况！\n【温馨提示2】如你的查询项比别人少，说明你相关活动没有开启，请手动开启活动！\n【实名信息】`;
        if ($.isRealNameAuth)
            ReturnMessage += `已实名\n`;
        else
            ReturnMessage += `未实名\n`;
    ...
    if ($.joylevel || $.jdCash || $.JoyRunningAmount) {
        ...
        if ($.jdCash) {
            ...
            ReturnMessage += `${$.jdCash}元\n`;
        }
        if ($.JoyRunningAmount) {
            if ($.joylevel || $.jdCash) {
                ReturnMessage += "【汪汪赛跑】";
            }
            ReturnMessage += `${$.JoyRunningAmount}元`;
        }
    ...
    if ($.hfjifen)
        ReturnMessage += `【话费积分】${$.hfjifen}积分(≈${($.hfjifen / 1).toFixed(2)}元话费)\n`;

    if ($.diandianquan) {
        ReturnMessage += `【天天点券】${$.diandianquan}券(≈${($.diandianquan / 1000).toFixed(2)}元)\n`
    }
    if ($.ProductName) {
        ReturnMessage += `【签到免单】商品${$.ProductName},已经签到${$.HasSignDays}/${$.NeedSignDays}\n`
        if($.ProductName1) {
            ReturnMessage +=`商品${$.ProductName1},已经签到${$.HasSignDays1}/${$.NeedSignDays1}\n`
        }
    }
	
    
    if ($.PlantBean) {
        ReturnMessage += `【种豆得豆】${$.PlantBean}成长值,累积获取京豆${$.RewardCenter},${$.PlantBeantime}\n`
    }
     if ($.Health) {
        ReturnMessage += `【健康社区】${$.Health}能量\n`
    }

    if ($.Mofang) {
        ReturnMessage += `【收集魔方】${$.Mofang}个\n`
    }

    if ($.jxgolds) {
        ReturnMessage += `【京喜购物】${$.jxgolds}金\n`
    }
    ...
    if ($.JdFarmProdName != "") {
        if ($.JdtreeEnergy != 0) {
            if ($.treeState === 2 || $.treeState === 3) {
                ...
                allReceiveMessage += `您的用户：${$.nickName || $.UserName}\r东东农场的${$.JdFarmProdName}已经可以兑换啦 \n`;
            } else {
                if ($.JdwaterD != 'Infinity' && $.JdwaterD != '-Infinity') {
                    ReturnMessage += `【东东农场】${$.JdFarmProdName}(${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%,${$.JdwaterD}天),共种值${$.JdwinTimes}次,已浇水${$.farmInfo.farmUserPro.treeEnergy / 10}次,还需${($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10}次\n`;
                } else {
                    ReturnMessage += `【东东农场】${$.JdFarmProdName}(${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%),共种值${$.JdwinTimes}次,已浇水${$.farmInfo.farmUserPro.treeEnergy / 10}次,还需${($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10}次\n`;
                }
            }
        } else {
            if ($.treeState === 0) {
                TempBaipiao += `【东东农场】水果领取后未重新种植!\n`;
                allWarnMessage += `您的用户：${$.nickName || $.UserName}/r水果领取后未重新种植! (东东农场)\n`;

            } else if ($.treeState === 1) {
                ReturnMessage += `【东东农场】${$.JdFarmProdName}种植中,共种值${$.JdwinTimes}次,已浇水${$.farmInfo.farmUserPro.treeEnergy / 10}次,还需${($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10}次\n\n`;
            } else {
                TempBaipiao += `【东东农场】状态异常!\n`;
                allWarnMessage += `您的用户：${$.nickName || $.UserName}/r状态异常! (东东农场)\n`;
            }
        }
    }
    if ($.jxFactoryInfo) {
    ...
    if ($.jxFactoryReceive) {
        allReceiveMessage += `【账号${$.nickName || $.UserName}】${$.jxFactoryReceive} (京喜工厂)\n`;
        TempBaipiao += `您的用户：${$.nickName || $.UserName}\r(京喜工厂的${$.jxFactoryReceive}已经可以兑换啦 \n`;
    }
    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
        ...
        if ($.petInfo.userStatus === 0) {
            ...
            allReceiveMessage += `您的用户：${$.nickName || $.UserName}\r东东萌宠的${$.petInfo.goodsInfo.goodsName}已经可以兑换啦 \n`;
        } else if ($.petInfo.petStatus === 6) {
            TempBaipiao += `【东东萌宠】未选择物品! \n`;
            allWarnMessage += `您的用户：${$.nickName || $.UserName}/r未选择物品! (东东萌宠)\n`;
            ...
            allWarnMessage += `您的用户：${$.nickName || $.UserName}/r暂未选购新的商品! (东东萌宠)\n`;
        }
    }

    if (strGuoqi) {
        ReturnMessage += `【临期京豆】${strGuoqi}\n`;
...
//种豆得豆累积
async function rewardCenter() {
    let UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`;
    let opts = {
        url: `https://api.m.jd.com/client.action?functionId=playUser&client=wh5&clientVersion=1.0.0`,
        headers: {
            'cookie': cookie,
            'user-agent': UA,
            'referer': 'https://funearth.m.jd.com/',
            'content-type': 'application/x-www-form-urlencoded',

        },
        body: "body=%7B%22client%22%3A%22app%22%2C%22whImg%22%3A%22%22%2C%22nickName%22%3A%22%22%7D",
    }
    let data = await reqpost(opts)
    try {
        const result = JSON.parse(data)
        // console.log(result)
        if (result.code == 0) {
            $.RewardCenter = result.data.jbean;
        }
    } catch (e) {
        //$.logErr(e);
    }
...
//种豆成长值查询
async function plantBean() {
    let UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`;
    let opts = {
        url: `https://api.m.jd.com/client.action?functionId=plantBeanIndex&body=%7B%22monitor_source%22%3A%22plant_m_plant_index%22%2C%22monitor_refer%22%3A%22%22%2C%22version%22%3A%229.2.4.2%22%7D&appid=ld&client=android&clientVersion=11.2.5&networkType=UNKNOWN&osVersion=9&uuid=${$.UUID}`,
        headers: {
            'cookie': cookie,
            'user-agent': UA,
    ...
    try {
        const result = JSON.parse(data)
        //console.log(result.data.growth)
        if (result.code == 0) {
            $.PlantBean = result.data.roundList[1].growth;
            $.PlantBeantime = result.data.roundList[1].dateDesc
        }
    } catch (e) {
        //$.logErr(e);
    }
...
async function mofang() {
    let opts = {
        url: `https://api.m.jd.com/client.action?functionId=queryInteractiveRewardInfo&client=wh5&clientVersion=1.0.0&appid=content_ecology&area=19_1643_36176_36187`,
        headers: {
            'cookie': cookie,
            'user-agent': 'jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148',
            'referer': 'https://h5.m.jd.com/',
        ...
        body: "body=%7B%22encryptProjectId%22%3A%223pp3mvzmgcFm7mvU3S1wZihNKi1H%22%2C%22sourceCode%22%3A%22acexinpin0823%22%2C%22ext%22%3A%7B%22needExchangeRestScore%22%3A%221%22%7D%7D",
    }
    let data = await reqpost(opts)
    try {
        const result = JSON.parse(data)
        //console.log(result.exchangeRestScoreMap['367'])
        if (result.code == 0) {
            $.Mofang = result.exchangeRestScoreMap['367'];
        }
    } catch (e) {
        //$.logErr(e);
    }
                        ...
                        if (data.ret === 0) {
                            data['expirejingdou'].map(item => {
                                if (item['expireamount'] != 0) {
                                    strGuoqi += `${timeFormat(item['time'] * 1000)}日,过期${item['expireamount']}豆\n`;
                                    if (decExBean == 0)
...
//签到免单
async function signFreeHome() {
    let t = new Date().getTime()
    let UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148`;
    let opts = {
        url: `https://api.m.jd.com/?functionId=signFreeHome&body={"linkId":"PiuLvM8vamONsWzC0wqBGQ"}&t=${t}&appid=activities_platform`,
        headers: {
            'cookie': cookie,
            'user-agent': UA,
    ...
    try {
        const result = JSON.parse(data)
        if (result.code == 0) {
            $.HasSignDays = result.data.signFreeOrderInfoList[0].hasSignDays; //免单签到天数
            $.ProductName = result.data.signFreeOrderInfoList[0].productName; //免单商品名字
            $.NeedSignDays = result.data.signFreeOrderInfoList[0].needSignDays; //免单需要签到天数
            $.HasSignDays1 = result.data.signFreeOrderInfoList[1].hasSignDays; //免单签到天数
            $.ProductName1 = result.data.signFreeOrderInfoList[1].productName; //免单商品名字
            $.NeedSignDays1 = result.data.signFreeOrderInfoList[1].needSignDays; //免单需要签到天数
            $.SumTotalFreeAmount = result.data.sumTotalFreeAmount; //总免单金额
        } 
    } catch (e) {
        //$.logErr(e);
    }
...
async function health() {
    ...
    try {
        const result = JSON.parse(data)
        //console.log(result.data.result.userScore)
        if (result.data.bizCode == 0) {
            $.Health = result.data.result.userScore;
        }
    } catch (e) {
        //$.logErr(e);
    }
                ...
                if (err) {
                    ...
                    if (data) {
                        ...
                        for (let vo of data.useRedInfo.redList || []) {
                            ...
                            } else if (vo.activityName.includes("京东特价APP") || vo.activityName.includes("特价版")) {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if ($.farmInfo.farmUserPro) {
                            ...
                            $.JdwinTimes = $.farmInfo.farmUserPro.winTimes;
                ...
                try {
                    if (err) {
                        ...
                        if (safeGet(data)) {
                            ...
                            //console.log($.JDwaterEveryDayT)
                            console.log(data)
                        }
                    }
                } catch (e) {
                    //$.logErr(e, resp);
                    console.log("出现运行异常状态")
                }
                finally {
                    resolve(data);
                }
...
async function getjdfruitinfo() {
    ...
    /* 
     await getProxy();
     global.GLOBAL_AGENT.HTTPS_PROXY = "http://" + proxy;
...
/*function getProxy() {
    return new Promise(async (resolve) => {
        $.get(
            {
                url: "http://api2.xkdaili.com/tools/XApi.ashx?apikey=XK12C267356D53671750&qty=1&format=txt&split=0&sign=1cc681d8f396bca432d6a3230d04ed43",
                timeout: {
                    request: 2000,
                },
            },
